    ...
    <div class="main">
        <div class="Users Dashboard">
            ...
            <template v-for="user in filteredUsers" :key="user.id">
                <!-- <div class="details" v-for="item in data" :key="item.id"> -->
...
<script setup>
import TestModal2 from "./Test/TestModal2.vue";
...
const deleteuser = async (user) => {
    const userConfirmed = confirm(`Bạn có chắc chắn muốn xoá ${user.name} không?`);
    if (userConfirmed) {
        try {
            ...
            if (response.status === 200) {
                alert("Xoá " + user.name + " thành công!");
            } else {
                alert("Xoá " + user.name + " thất bại!");
            }
        } catch (error) {
            alert("Có lỗi xảy ra khi xoá " + user.name);
        }
        fetchData();
    } else {
        alert("Hành động xoá đã bị huỷ.");
    }
};
